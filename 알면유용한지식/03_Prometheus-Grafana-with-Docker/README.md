...
## 🎁 목차
- [Prometheus Grafana with Docker](#prometheus-grafana-with-docker)
  - [🎁 목차](#-목차)
  - [0. 개요](#0-개요)
  - [1. Pull docker image](#1-pull-docker-image)
  - [2. Create prometheus yml](#2-create-prometheus-yml)
  - [3. Set defaults.ini](#3-set-defaultsini)
  - [4. Create docker-compose.yml](#4-create-docker-composeyml)
  - [5. Start](#5-start)
  - [6. Grafana setting](#6-grafana-setting)
## 0. 개요
주기적으로 쌓이는 metric data를 모니터링하는 방법 중 하나를 소개한다.
현 예제에서 환경은 다음과 같다.
 - Metric data: `Spring-boot`
 - Time series DB: `Prometheus`
 - Monitoring tool: `Grafana`
 
Spring-boot에서 metric data를 제공한다고 가정한다.
(이 부분이 궁금하다면 검색해보자. 어렵지 않다 😅)

## 1. Pull docker image
Prometheus와 Grafana docker image를 준비한다.
...
Prometheus에서 전달받을 spring-boot 정보를 입력한다.
...
## 3. Set defaults.ini
그라파나에서 사용할 `defaults.ini`를 작성한다.
```ini
# defaults.ini
...
Prometheus와 Grafana를 실행할 `docker-compose.yaml`을 작성한다.
...
## 6. Grafana setting
Grafana에서 Prometheus의 data들을 가져오고, template를 활용하고자 한다.
